...
class Conf(object):
    def __init__(self, data_name, param_dict=None):
        ...
        self.num_filters = [50]
        self.filter_lengths = [[1, 3, 5]]
        self.poolings = ['average']
        self.pool_lengths = [-1]
        self.conv_dropout_rate = 0.3
...
def get_conf_best(data_name, param_dict=None):
    ...
    if data_name.startswith('citeulike_title_only'):
        ...
        conf.filter_lengths = [3]
        conf.conv_dropout_rate = 0.1
        ...
        conf.filter_lengths = [3]
        conf.conv_dropout_rate = 0.1
        conf.item_dense_transform['dense_dropout_rate'] = 0.
    else:
...
def get_conf_random(data_name, param_dict=None):
    ...
    #conf.pretrain = None
    u_reg_list = [1e-6]
    pretrain_combine_dropout_list = [0.3]
    filter_lengths_list = [[3]]
    num_filters_list = [[50]]
    #word_emb_dropout_rate_list = [0.3, 0.5]
    #conv_dropout_rate_list = [0.1, 0.3]
    word_emb_dropout_rate_list = [0.5]
    conv_dropout_rate_list = [0.1, 0.3]
    item_dense_dropout_list = [0.1, 0.3]

    import itertools    
    conf_i = int(conf_var)
    configs = [each for each in itertools.product( \
        u_reg_list, word_emb_dropout_rate_list, \
        pretrain_combine_dropout_list, filter_lengths_list, num_filters_list, \
        conv_dropout_rate_list, item_dense_dropout_list)]
    conf.u_reg, conf.word_emb_dropout_rate, \
        conf.pretrain['pretrain_combine_dropout'], conf.filter_lengths, \
        conf.num_filters, conf.conv_dropout_rate, \
        conf.item_dense_transform['dense_dropout_rate'] = configs[conf_i]
    # conf.pretrain = None
    return conf
